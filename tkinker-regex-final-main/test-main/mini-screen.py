...
# from miniscreen2 import application
...
def fenetreconnexion():
    ...
    op1=Menu(menu_gui,tearoff=0)
    menu_gui.add_cascade(label='options',menu=op)
    menu_gui.add_cascade(label='connectez-vous',menu=op1)
    # menu_gui.add_command(label='connectez vous',command=lambda:[fenetrelogin(),quit_log()])
    op1.add_command(label='connexion',command=lambda:[fenetrelogin(),quit_log()])
...
def fenetrelogin():
    ...
    op.add_command(label='contact us')
    op.add_command(label='help')
    op.add_command(label='inscrivez-vous',command=lambda:[fenetreconnexion(),quit_log1()])
